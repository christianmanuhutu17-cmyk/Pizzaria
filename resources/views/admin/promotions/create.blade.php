...
<div style="max-width: 750px;">
    ...
    <div class="card">
        <div style="padding: 20px 25px; border-bottom: 1px solid var(--border-color);">
            <h2 style="font-size: 1.3rem; font-weight: 800; color: var(--text-main);">Buat Promosi Baru</h2>
            <p style="color: var(--text-muted); font-size: 0.95rem; margin-top: 5px;">Atur kode kupon, tipe diskon, dan batasan penggunaan.</p>
        </div>
        
        <form action="{{ route('admin.promotions.store') }}" method="POST" enctype="multipart/form-data" style="padding: 25px;">
            @csrf
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" placeholder="e.g., DISKON20" required style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: 'Courier New', monospace; font-size: 1.1rem; text-transform: uppercase; letter-spacing: 1px;">
                </div>
                ...
                <div style="grid-column: span 2;">
                    ...
                    <input type="text" name="description" value="{{ old('description') }}" placeholder="e.g., Diskon 20% khusus pengguna baru" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="discount_value" value="{{ old('discount_value') }}" placeholder="e.g., 20" required step="0.01" min="0" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                    <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: 4px;">Jika persentase, isi angka persen (mis: 20 = 20%). Jika nominal, isi jumlah Rupiah.</p>
                </div>
                ...
                <div id="maxDiscountGroup">
                    ...
                    <input type="number" name="max_discount" value="{{ old('max_discount') }}" placeholder="e.g., 50000" min="0" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                    <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: 4px;">Khusus tipe persentase. Batas atas diskon.</p>
                </div>

                <div>
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Min. Order (Rp)</label>
                    <input type="number" name="min_order_amount" value="{{ old('min_order_amount', 0) }}" min="0" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>

                <div>
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Batas Pemakaian</label>
                    <input type="number" name="usage_limit" value="{{ old('usage_limit') }}" placeholder="Kosongkan = tak terbatas" min="1" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>

                <div>
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Mulai Berlaku</label>
                    <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>

                <div>
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Berakhir Pada</label>
                    <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>
                ...
                <div style="grid-column: span 2;">
                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; padding: 15px; border: 2px solid var(--border-color); border-radius: 8px; background: #fafafa; margin-bottom: 10px;">
                        <input type="checkbox" name="is_first_order_only" value="1" {{ old('is_first_order_only') ? 'checked' : '' }} style="width: 20px; height: 20px; accent-color: var(--primary);">
                        <div>
                    ...
                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; padding: 15px; border: 2px solid var(--border-color); border-radius: 8px; background: #fafafa;">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} style="width: 20px; height: 20px; accent-color: var(--primary);">
                        <div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px; background: #f8f9fa; padding: 20px; border-radius: 8px; border: 1px dashed var(--border-color);">
                <div style="grid-column: span 2;">
                    ...
                    <input type="text" name="banner_title" value="{{ old('banner_title') }}" placeholder="e.g., PESTA BOLA 2026!" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>
                <div style="grid-column: span 2;">
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Sub-judul / Tagline</label>
                    <input type="text" name="banner_subtitle" value="{{ old('banner_subtitle') }}" placeholder="e.g., Nobar makin seru ditemani Pizza hangat..." style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>
                <div>
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Warna Tema (Hex Code)</label>
                    <input type="color" name="theme_color" value="{{ old('theme_color', '#E8304A') }}" style="width: 100%; height: 50px; padding: 5px; border: 2px solid var(--border-color); border-radius: 8px; cursor: pointer;">
                </div>
                <div>
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Ikon (FontAwesome Class)</label>
                    <input type="text" name="icon" value="{{ old('icon', 'fa-bolt') }}" placeholder="e.g., fa-futbol" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                ...
                <div style="grid-column: span 2;">
                    ...
                    <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: 4px;">Disarankan dimensi melebar (landscape) dengan warna dominan gelap agar teks terlihat.</p>
                </div>
            </div>
            
            <div style="display: flex; gap: 15px; margin-top: 30px;">
                <button type="submit" class="btn-primary" style="flex: 1; padding: 14px; font-size: 1.05rem;"><i class="fa-solid fa-save"></i> Simpan Promo</button>
