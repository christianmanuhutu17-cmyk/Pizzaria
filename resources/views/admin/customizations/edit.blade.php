...
@section('title', 'Edit Kustomisasi')
@section('content')
<div style="max-width: 800px;">
    ...
    <div class="card" style="padding: 25px;">
        <div style="border-bottom: 1px solid var(--border-color); margin-bottom: 25px; padding-bottom: 15px;">
            <h2 style="font-size: 1.3rem; font-weight: 800; color: var(--text-main);">Edit Opsi Kustomisasi</h2>
            <p style="color: var(--text-muted); font-size: 0.95rem; margin-top: 5px;">Perbarui data pilihan tambahan dan relasinya dengan bahan baku.</p>
        </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div style="grid-column: span 2;">
                    ...
                    <div style="display: flex; gap: 20px;">
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="radio" name="target_type" value="global" onchange="toggleTarget()" {{ old('target_type', $target_type) == 'global' ? 'checked' : '' }} required> Semua Menu (Global)
                        </label>
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="radio" name="target_type" value="category" onchange="toggleTarget()" {{ old('target_type', $target_type) == 'category' ? 'checked' : '' }}> Kategori Tertentu
                        </label>
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="radio" name="target_type" value="menu" onchange="toggleTarget()" {{ old('target_type', $target_type) == 'menu' ? 'checked' : '' }}> Menu Spesifik
                        </label>
                    </div>
                </div>
                ...
                <div id="category_select" style="display: none;">
                    ...
                    <select name="category_id" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem; background: white;">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ (old('category_id') ?? $customization->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="menu_select" style="display: none;">
                    ...
                    <select name="menu_id" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem; background: white;">
                        ...
                        @foreach($menus as $menu)
                            <option value="{{ $menu->id }}" {{ (old('menu_id') ?? $customization->menu_id) == $menu->id ? 'selected' : '' }}>{{ $menu->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="type" required style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem; background: white;">
                        <option value="topping" {{ (old('type') ?? $customization->type) == 'topping' ? 'selected' : '' }}>Topping</option>
                        <option value="size" {{ (old('type') ?? $customization->type) == 'size' ? 'selected' : '' }}>Size (Ukuran)</option>
                        <option value="crust" {{ (old('type') ?? $customization->type) == 'crust' ? 'selected' : '' }}>Crust (Pinggiran)</option>
                    </select>
                </div>
                ...
                <div style="grid-column: span 2;">
                    ...
                    <input type="text" name="name" value="{{ old('name', $customization->name) }}" required style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>

                <div style="grid-column: span 2;">
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: var(--text-main);">Tambahan Harga (Rp)</label>
                    <input type="number" name="additional_price" value="{{ old('additional_price', $customization->additional_price) }}" required min="0" style="width: 100%; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                </div>
                ...
                <div style="grid-column: span 2; background: #fafafa; padding: 20px; border: 1px solid var(--border-color); border-radius: 8px;">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock" value="{{ old('stock', $customization->stock) }}" min="0" placeholder="Misal: 100" style="width: 100%; max-width: 300px; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                    </div>
                    ...
                    <div style="margin-top: 15px;">
                        ...
                        <select name="deduct_ingredient_id" style="width: 100%; max-width: 300px; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem; background: white;">
                            ...
                            @foreach($ingredients as $ingredient)
                                <option value="{{ $ingredient->id }}" {{ (old('deduct_ingredient_id') ?? $customization->deduct_ingredient_id) == $ingredient->id ? 'selected' : '' }}>{{ $ingredient->name }} ({{ $ingredient->unit }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div style="margin-top: 15px;">
                        ...
                        <input type="number" step="0.01" name="deduct_qty" value="{{ old('deduct_qty', $customization->deduct_qty) }}" min="0" placeholder="Misal: 0.5" style="width: 100%; max-width: 300px; padding: 12px 15px; border: 2px solid var(--border-color); border-radius: 8px; font-family: inherit; font-size: 1rem;">
                    </div>
                </div>
            </div>
            
            <button type="submit" class="btn-primary" style="margin-top: 25px; width: 100%; padding: 14px; font-size: 1.05rem;"><i class="fa-solid fa-save"></i> Simpan Perubahan</button>
        </form>
    </div>
</div>
</div>
